<style lang="scss">
	...
	.view-box-list-popup.ask-modal-box{
		.ask-modal-wrapper{
			width: 550px;
			padding: 0;
			border-radius: 8px;
			...
			<template v-if="!inlineLoaderShow || list.length > 0">
				...
				<div class="ul-table-body" @scroll="onScroll($event)">
					...
					<template v-for="(once,$i) in list">
						<ul class="ul-table">
							<li class="ask-col-100">{{once.name || '无'}}</li>
						</ul>
					</template>
